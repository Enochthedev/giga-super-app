...
async function handleSuccessfulPayment(supabase, data) {
  ...
  try {
    // Find payment record
    const { data: payment, error: paymentError } = await supabase.from('payments').select('*').eq('transaction_id', reference).single();
    ...
    const { data: commissionAmount, error: commissionError } = await supabase.rpc('calculate_commission', {
    ...
    let vendorType = '';
    if (payment.payment_type === 'hotel_booking') {
      const { data: booking } = await supabase.from('hotel_bookings').select('hotel:hotels(host_id)').eq('id', payment.reference_id).single();
      if (booking?.hotel) {
        vendorId = booking.hotel.host_id;
        vendorType = 'host';
      }
    } else if (payment.payment_type === 'ecommerce_order') {
      const { data: order } = await supabase.from('ecommerce_orders').select('items:ecommerce_order_items(vendor_id)').eq('id', payment.reference_id).single();
      if (order?.items?.[0]) {
        vendorId = order.items[0].vendor_id;
        vendorType = 'merchant';
      }
    } else if (payment.payment_type === 'taxi_ride') {
      const { data: ride } = await supabase.from('rides').select('driver_id').eq('id', payment.reference_id).single();
      if (ride) {
        vendorId = ride.driver_id;
        vendorType = 'driver';
      }
    } else if (payment.payment_type === 'ad_campaign') {
      const { data: campaign } = await supabase.from('ad_campaigns').select('advertiser_id').eq('id', payment.reference_id).single();
      if (campaign) {
    ...
    let escrowId = null;
    if (vendorId) {
      const { data: escrow } = await supabase.from('escrow_transactions').insert({
...
async function handleFailedPayment(supabase, data) {
  ...
  try {
    // Find payment record
    const { data: payment } = await supabase.from('payments').select('*').eq('transaction_id', reference).single();
    if (!payment) {
