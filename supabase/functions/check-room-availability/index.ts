...
serve(async (req)=>{
  ...
  try {
    ...
    // Get hotel info
    const { data: hotel, error: hotelError } = await supabaseClient.from('hotels').select('id, name, is_active, is_verified').eq('id', params.hotelId).single();
    ...
    const { data: roomTypes, error: roomTypesError } = await supabaseClient.from('room_types').select('*').eq('hotel_id', params.hotelId).eq('is_active', true).order('display_order', {
      ascending: true
    });
    ...
    for (const roomType of roomTypes){
      ...
      ;
      try {
        const { data: availabilityData, error: availError } = await supabaseClient.rpc('get_available_room_count', {
        ...
        if (!availError && availabilityData !== null) {
          ...
          const { data: availRecords } = await supabaseClient.from('room_availability').select('available_rooms').eq('room_type_id', roomType.id).gte('date', params.checkInDate).lt('date', params.checkOutDate);
      ...
      if (availableCount >= roomsNeeded) {
        // Calculate pricing
        const { totalPrice, priceBreakdown } = await calculateRoomPrice(supabaseClient, roomType, params.checkInDate, params.checkOutDate, roomsNeeded);
        availableRooms.push({
...
async function calculateRoomPrice(supabase, roomType, checkInDate, checkOutDate, roomsCount) {
  const checkIn = new Date(checkInDate);
  const checkOut = new Date(checkOutDate);
  // Get availability records with pricing for date range
  const { data: availability, error } = await supabase.from('room_availability').select('date, base_price, dynamic_price').eq('room_type_id', roomType.id).gte('date', checkInDate).lt('date', checkOutDate).order('date', {
    ascending: true
  });
  if (error) {
  ...
  const avgPricePerNight = pricePerNight.reduce((sum, price)=>sum + price, 0) / pricePerNight.length;
  return {
    totalPrice: subtotal,
    priceBreakdown: {
