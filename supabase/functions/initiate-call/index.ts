...
Deno.serve(async (req: Request) => {
  try {
    ...
    if (!participant) {
      return new Response(JSON.stringify({ error: "Not a participant of this conversation" }), { status: 403 });
    }
    ...
    const agoraToken = generateAgoraToken(channelName, parseInt(user.id.replace(/-/g, '').substring(0, 8), 16));
