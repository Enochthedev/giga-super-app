...
serve(async (req) => {
    ...
    try {
        ...
        const { pickup_lat, pickup_lng, dropoff_lat, dropoff_lng, vehicle_type = 'standard' } = await req.json();
        ...
        let estimated_fare = (base_fare + distance_cost + time_cost) * surge_multiplier * vehicle_multiplier;
        ...
        const vehicle_options = allVehicleTypes?.map((vt) => {
            const fare = Math.round(
                ((base_fare + distance_cost + time_cost) * surge_multiplier * vt.price_multiplier) / 50
            ) * 50;
            return {
                type: vt.slug,
                name: vt.name,
            ...
            JSON.stringify({
                ...
                data: {
                    ...
                    fare_breakdown: {
                        ...
                        surge: surge_multiplier > 1.0 ? Math.round((estimated_fare / surge_multiplier) * (surge_multiplier - 1)) : 0,
                    },
                },
            }),
            {
