...
serve(async req => {
  ...
  try {
    ...
    let query = supabaseClient
      .from('ad_campaigns')
      .select(
        'id, campaign_name, impressions, clicks, conversions, ctr, spent_amount, budget, status, start_date, end_date'
      )
    ...
    const analytics = data.map(campaign => ({
      ...
      cpm:
        campaign.impressions > 0
          ? ((campaign.spent_amount / campaign.impressions) * 1000).toFixed(2)
          : 0,
    }));
