  ...
  try {
    ...
    if (cart_id) {
      const cartResp = await fetch(
        `${supabaseUrl}/rest/v1/ecommerce_carts?id=eq.${encodeURIComponent(cart_id)}&select=*`,
        {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
    ...
    if (!cartData || (Array.isArray(cartData) && cartData.length === 0)) {
      const byUserResp = await fetch(
        `${supabaseUrl}/rest/v1/ecommerce_carts?user_id=eq.${userId}&select=*`,
        {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
      ...
      const txt2 =
        byUserResp.status === 200
          ? await byUserResp.text().catch(() => '')
          : await byUserResp.text();
      await log({
    ...
    if (!cartData || (Array.isArray(cartData) && cartData.length === 0)) {
      const svcResp = await fetch(
        `${supabaseUrl}/rest/v1/ecommerce_carts${cart_id ? `?id=eq.${encodeURIComponent(cart_id)}` : `?user_id=eq.${userId}`}&select=*`,
        {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
    ...
    let finalCart = null;
    if (
      (!serviceCart || (Array.isArray(serviceCart) && serviceCart.length === 0)) &&
      (!cartData || (Array.isArray(cartData) && cartData.length === 0))
    ) {
      const payload = {
        user_id: userId,
