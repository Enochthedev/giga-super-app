...
serve(async req => {
  ...
  try {
    ...
    if (driverError || !driverProfile) {
      throw new Error(
        'Driver profile not found. Only verified drivers can accept rides.'
      );
    }
    ...
    if (!driverProfile.is_available) {
      throw new Error(
        'You must be online to accept rides. Toggle your availability first.'
      );
    }
    ...
    if (ride.scheduled_time) {
      ...
      if (hoursUntil > 1) {
        throw new Error(
          `This ride is scheduled for ${scheduledTime.toLocaleString()}. Too early to accept.`
        );
      }
    }
    ...
    if (updateError) {
      ...
      throw new Error(
        'Failed to accept ride. It may have been accepted by another driver.'
      );
    }
