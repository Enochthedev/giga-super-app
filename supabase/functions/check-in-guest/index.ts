...
serve(async (req)=>{
  ...
  try {
    ...
    if (booking.payment_status !== 'paid') {
      throw new Error('Payment must be completed before check-in. Current status: ' + booking.payment_status);
    }
    ...
    if (!assignedRoomId) {
      ...
      const { data: availableRoom } = await supabaseClient.from('rooms').select('id').eq('room_type_id', booking.room_type_id).eq('status', 'available').limit(1).single();
      if (!availableRoom) {
    ...
    // Update room status
    const { error: roomUpdateError } = await supabaseClient.from('rooms').update({
      status: 'occupied',
