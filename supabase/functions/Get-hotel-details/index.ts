...
serve(async (req) => {
  ...
  try {
    ...
    const userLat = url.searchParams.get('lat') ? parseFloat(url.searchParams.get('lat')!) : null;
    const userLng = url.searchParams.get('lng') ? parseFloat(url.searchParams.get('lng')!) : null;
    ...
    if (userLat && userLng && hotel.latitude && hotel.longitude) {
      ...
      const a = Math.sin(dLat / 2) * Math.sin(dLat / 2) + Math.cos(toRad(userLat)) * Math.cos(toRad(hotel.latitude)) * Math.sin(dLon / 2) * Math.sin(dLon / 2);
    ...
    const photosByType = {
      ...
      interior: hotel.hotel_photos.filter((p) => p.photo_type === 'interior').sort((a, b) => a.display_order - b.display_order),
      exterior: hotel.hotel_photos.filter((p) => p.photo_type === 'exterior').sort((a, b) => a.display_order - b.display_order),
      amenity: hotel.hotel_photos.filter((p) => p.photo_type === 'amenity').sort((a, b) => a.display_order - b.display_order),
      restaurant: hotel.hotel_photos.filter((p) => p.photo_type === 'restaurant').sort((a, b) => a.display_order - b.display_order),
      all: hotel.hotel_photos.sort((a, b) => a.display_order - b.display_order)
    };
    // Get amenities from amenity mappings
    const { data: amenityMappings } = await supabaseClient.from('hotel_amenity_mappings').select(`
    ...
    const reviewsWithUsers = await Promise.all(hotel.hotel_reviews.filter((r) => r.is_approved).sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime()).slice(0, 10).map(async (review) => {
      // Fetch user profile
      const { data: userProfile } = await supabaseClient.from('user_profiles').select('first_name, last_name, avatar, avatar_url').eq('id', review.user_id).single();
      return {
        ...review,
        user: userProfile ? {
          ...
          display_name: `${userProfile.first_name} ${userProfile.last_name}`.trim() || 'Anonymous'
        } : {
          first_name: '',
          last_name: '',
          avatar: null,
    ...
    const minRoomPrice = hotel.room_types.length > 0 ? Math.min(...hotel.room_types.map((rt) => rt.base_price)) : null;
    ...
    const response = {
      ...
      data: {
        ...
        room_types: hotel.room_types.filter((rt) => rt.is_active).sort((a, b) => a.display_order - b.display_order).map((rt) => ({
          ...rt,
          photos: rt.images || []
        })),
