...
serve(async (req)=>{
  ...
  try {
    const supabaseClient = createClient(Deno.env.get('SUPABASE_URL') ?? '', Deno.env.get('SUPABASE_SERVICE_ROLE_KEY') ?? '' // Service role for host actions
    );
    ...
    // Get booking details
    const { data: booking, error: bookingError } = await supabaseClient.from('hotel_bookings').select(`
        *,
        hotel:hotels(
          id,
          name,
    ...
    if (booking.hotel.host_id !== user.id) {
      // Check if admin
      const { data: roles } = await supabaseClient.from('user_roles').select('role_name').eq('user_id', user.id).eq('role_name', 'ADMIN');
    ...
    if (booking.booking_status !== 'checked_in' && booking.booking_status !== 'confirmed') {
      throw new Error('Guest must be checked in first');
    }
    // Update booking status
    const { error: updateError } = await supabaseClient.from('hotel_bookings').update({
    ...
    const { data: escrowRelease, error: escrowError } = await supabaseClient.rpc('release_escrow_atomic', {
    ...
    return new Response(JSON.stringify({
      ...
      data: {
        ...
        escrow_details: escrowError ? null : {
