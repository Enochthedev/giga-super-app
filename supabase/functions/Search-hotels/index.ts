...
serve(async (req)=>{
  ...
  try {
    ...
    if (params.userLat && params.userLng) {
      ...
      hotelsWithDistance = hotels.map((hotel)=>{
        ...
        const a = Math.sin(dLat / 2) * Math.sin(dLat / 2) + Math.cos(toRad(params.userLat)) * Math.cos(toRad(hotel.latitude)) * Math.sin(dLon / 2) * Math.sin(dLon / 2);
      ...
      // Search by city name
      const { data: hotels, error: hotelsError } = await query.ilike('city', `%${params.city}%`);
      if (hotelsError) throw hotelsError;
    ...
    if (params.starRating && params.starRating.length > 0) {
      hotelsWithDistance = hotelsWithDistance.filter((hotel)=>params.starRating.includes(hotel.star_rating));
    }
    // Filter by amenities
    ...
    if (params.checkInDate && params.checkOutDate) {
      const availableHotels = await checkAvailability(supabaseClient, hotelsWithDistance, params.checkInDate, params.checkOutDate, params.rooms || 1);
...
async function checkAvailability(supabase, hotels, checkIn, checkOut, roomsNeeded) {
  ...
  for (const hotel of hotels){
    ...
    const { data: availability, error } = await supabase.from('room_availability').select('room_type_id, available_rooms').in('room_type_id', roomTypeIds).gte('date', checkIn).lt('date', checkOut).eq('is_blocked', false);
    if (error) continue;
    ...
    const hasAvailability = Object.values(roomTypeAvailability).some((rooms)=>rooms.every((count)=>count >= roomsNeeded));
    if (hasAvailability) {
