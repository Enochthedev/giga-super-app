  ...
  try {
    ...
    bookings?.forEach(booking => {
      ...
      const enhancedBooking = {
        ...
        // Status indicators
        is_upcoming:
          checkIn > now && ['pending', 'confirmed'].includes(booking.booking_status),
        is_ongoing:
          todayStr >= booking.check_in_date && todayStr < booking.check_out_date,
        is_past: checkOut < now,
        is_cancelled: ['cancelled', 'no_show'].includes(booking.booking_status),
        // Time calculations
        days_until_checkin: Math.ceil(
