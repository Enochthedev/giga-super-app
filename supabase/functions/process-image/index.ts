...
serve(async (req)=>{
  ...
  try {
    ...
    // Get file metadata
    const { data: fileMetadata, error: fetchError } = await supabaseClient.from('file_metadata').select('*').eq('id', fileId).single();
    if (fetchError) {
    ...
    await supabaseClient.from('file_metadata').update({
      status: 'processing'
    }).eq('id', fileId);
    const bucket = fileMetadata.access_level === 'public' ? 'public-files' : 'private-files';
    console.log('Downloading from bucket:', bucket, fileMetadata.storage_path);
    // Download original file
    const { data: fileData, error: downloadError } = await supabaseClient.storage.from(bucket).download(fileMetadata.storage_path);
    if (downloadError) {
    ...
    for (const op of operations){
      try {
        ...
        if (op.type === 'thumbnail') {
          ...
          const { error: uploadError } = await supabaseClient.storage.from(bucket).upload(thumbPath, encoded, {
          ...
          const { data: urlData } = supabaseClient.storage.from(bucket).getPublicUrl(thumbPath);
        ...
        if (op.type === 'resize' && op.width && op.height) {
          ...
          const { error: uploadError } = await supabaseClient.storage.from(bucket).upload(resizedPath, encoded, {
            contentType: 'image/jpeg',
            cacheControl: '3600',
            upsert: true
          });
          ...
          const { data: urlData } = supabaseClient.storage.from(bucket).getPublicUrl(resizedPath);
        ...
        if (op.type === 'compress') {
          ...
          const { error: uploadError } = await supabaseClient.storage.from(bucket).upload(compressedPath, compressed, {
            contentType: 'image/jpeg',
            cacheControl: '3600',
            upsert: true
          });
          ...
          const { data: urlData } = supabaseClient.storage.from(bucket).getPublicUrl(compressedPath);
    ...
    const { error: updateError } = await supabaseClient.from('file_metadata').update({
