...
serve(async (req)=>{
  ...
  try {
    ...
    const { user_id, template_name, variables, recipient_email, recipient_phone, recipient_device_token, priority = 5, delay_seconds = 0 } = await req.json();
    // Validation
    ...
    const { data: queueEntry, error: queueError } = await supabaseClient.from('notification_queue').insert({
      user_id,
      template_name,
      variables,
      recipient_email,
