    ...
    try {
        ...
        const roomPerformance = await Promise.all(
            (roomTypes || []).map(async (rt) => {
                ...
                const revenue = roomBookings?.reduce((sum, b) => sum + (b.total_amount || 0), 0) || 0;
        ...
        const totalRooms = roomTypes?.reduce((sum, rt) => sum + (rt as any).total_rooms || 0, 0) || 0;
        ...
        const occupancyRate = totalRooms > 0 && totalDays > 0
