...
import { corsHeaders } from '../actions/create-booking.ts'; // Reuse header from sibling

export async function initializePayment(req: Request) {
  ...
  try {
    ...
    if (params.depositOnly) {
      const { data: depositCalc, error: depositError } = await supabaseClient.rpc(
        'calculate_deposit',
        {
    ...
    const useMockProvider =
      params.paymentProvider === 'mock' || Deno.env.get('USE_MOCK_PAYMENT') === 'true';
    console.log(
      `💳 Payment Provider: ${params.paymentProvider}, Mock Mode: ${useMockProvider}`
    );
    ...
    if (useMockProvider) {
      ...
      if (!providerConfig) {
        throw new Error(
          'Paystack not configured. Use paymentProvider: "mock" for testing.'
        );
      }
      ...
      throw new Error(
        `Unknown payment provider: ${params.paymentProvider}. Use "mock" for testing.`
      );
    }
    return new Response(
      JSON.stringify({
        ...
        data: {
          ...
          ...(useMockProvider && {
            test_instructions: {
              ...
              how_to_test:
                'Visit payment_url and click "Simulate Success" or "Simulate Failure"',
              webhook_test: `curl -X POST ${Deno.env.get('SUPABASE_URL')}/functions/v1/api/mock-payment-webhook -H "Content-Type: application/json" -d '{"transaction_id":"${transactionId}","status":"success"}'`, // Updated to router URL just in case
            },
          }),
...
async function initializePaystack(secretKey: string, data: any) {
