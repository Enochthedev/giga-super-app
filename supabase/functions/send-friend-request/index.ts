...
Deno.serve(async (req: Request) => {
  try {
    ...
    if (targetUserId === user.id) {
      return new Response(JSON.stringify({ error: "Cannot connect with yourself" }), { status: 400 });
    }
    ...
    if (existing) {
      return new Response(JSON.stringify({ error: "Connection already exists", status: existing.status }), { status: 400 });
    }
