...
Deno.serve(async (req: Request) => {
  try {
    ...
    else return new Response(JSON.stringify({ error: "Invalid content type" }), { status: 400 });
    ...
    if (action === "remove") {
      ...
      return new Response(JSON.stringify({ success: true, message: "Content removed" }), { status: 200 });
    }
    ...
    if (action === "restore") {
      ...
      if (error) throw error;
      return new Response(JSON.stringify({ success: true, message: "Content restored" }), { status: 200 });
    }
