...
Deno.serve(async (req: Request) => {
  try {
    ...
    if (!connectionId || !action) {
      return new Response(JSON.stringify({ error: "Connection ID and action required" }), { status: 400 });
    }
    ...
    if (action === "decline") {
      ...
      return new Response(JSON.stringify({ success: true, message: "Connection declined" }), { status: 200 });
    }
