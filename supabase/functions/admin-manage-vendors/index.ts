...
Deno.serve(async (req: Request) => {
  try {
    ...
    if (action === "verify") {
      ...
      return new Response(JSON.stringify({ success: true, message: "Vendor verified" }), { status: 200 });
    }
    ...
    if (action === "suspend") {
      ...
      if (error) throw error;
      return new Response(JSON.stringify({ success: true, message: "Vendor suspended" }), { status: 200 });
    }
