...
async function handleMockSuccess(supabase, params) {
  ...
  // Find payment record
  const { data: payment, error: paymentError } = await supabase.from('payments').select('*').eq('transaction_id', transaction_id).single();
...
async function handleMockFailure(supabase, params) {
  ...
  const { data: payment } = await supabase.from('payments').select('*').eq('transaction_id', transaction_id).single();
  if (!payment) {
    throw new Error('Payment record not found: ' + transaction_id);
  }
  await supabase.from('payments').update({
