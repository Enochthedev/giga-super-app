  ...
  try {
    if (route === 'add' && method === 'POST') {
      ...
      const variantIds = Array.from(
        new Set(itemsInput.filter(i => i.variant_id).map(i => i.variant_id))
      );
      let variantMap = new Map();
      if (variantIds.length) {
        const { data: variants } = await sb
          .from('ecommerce_product_variants')
          .select('id, price_adjustment, stock_quantity')
          .in('id', variantIds);
      ...
      for (const it of itemsInput) {
        ...
        if (it.variant_id && !variantMap.has(it.variant_id))
          return new Response(
            JSON.stringify({
