...
serve(async (req)=>{
  ...
  try {
    ...
    if (!params.hotelId || !params.roomTypeId || !params.checkInDate || !params.checkOutDate) {
    ...
    // Get room type info
    const { data: roomType, error: roomError } = await supabaseClient.from('room_types').select('*, hotel:hotels!inner(id, name, city, country)').eq('id', params.roomTypeId).eq('hotel_id', params.hotelId).single();
    ...
    while(currentDate < checkOut){
      ...
      const { data: availability } = await supabaseClient.from('room_availability').select('dynamic_price, base_price').eq('room_type_id', params.roomTypeId).eq('date', dateStr).single();
      let nightPrice;
      if (availability?.dynamic_price) {
        ...
        nightPrice = isWeekend && roomType.weekend_price ? parseFloat(roomType.weekend_price) : parseFloat(roomType.base_price);
      }
      pricePerNight.push({
        date: dateStr,
        price: nightPrice
    ...
    if (params.promoCode) {
      const promoResult = await validateAndApplyPromoCode(supabaseClient, params.promoCode, subtotal, params.hotelId, params.roomTypeId);
    ...
    const refundInfo = calculateRefundPolicy(roomType.refundable, roomType.cancellation_hours, totalAmount, params.checkInDate);
...
async function validateAndApplyPromoCode(supabase, code, subtotal, hotelId, roomTypeId) {
  const { data: promo, error } = await supabase.from('ecommerce_promo_codes').select('*').eq('code', code.toUpperCase()).eq('is_active', true).single();
  if (error || !promo) {
    return {
      valid: false,
