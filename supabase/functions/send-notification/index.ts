...
serve(async (req)=>{
  ...
  try {
    ...
    const supabaseClient = createClient(Deno.env.get('SUPABASE_URL') ?? '', Deno.env.get('SUPABASE_SERVICE_ROLE_KEY') ?? '' // Use service role for sending
    );
    const { user_id, template_name, variables, channels, recipient_email, recipient_phone, recipient_device_token } = await req.json();
    console.log('Request:', {
      user_id,
      template_name,
      channels
    ...
    // Get template
    const { data: template, error: templateError } = await supabaseClient.from('notification_templates').select('*').eq('name', template_name).eq('is_active', true).single();
    ...
    if (user_id) {
      const { data: prefs } = await supabaseClient.from('notification_preferences').select('*').eq('user_id', user_id).single();
    ...
    const { data: logEntry, error: logError } = await supabaseClient.from('notification_logs').insert({
      user_id,
