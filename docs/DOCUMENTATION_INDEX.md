...
Several API docs in this repo are stale and contradict each other. Ranked by
...
The Swagger UI URL is **`/api-docs/`** — with the trailing slash. `/docs/admin/`
is an infinite redirect (defect V9); raw JSON per service still works at
`/docs/{service}/json`.
...
**Start Here**: `../postman/collections/README.md`, then
`flows/NIPOST_ADMIN_FLOWS.md`
