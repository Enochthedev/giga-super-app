...
Supabase Realtime allows you to listen to database changes in real-time. Here's
how to use it:
...
The functions I created are **only in your local files**. They need to be
...
async function getSocialFeed(userId: string, limit: number, offset: number) {
  ...
  const scoredPosts = posts.map(post => {
    ...
    // Final score
    const score =
      engagementScore * timeDecay * friendBoost + Math.random() * 0.1; // Add randomness
