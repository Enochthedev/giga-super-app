...
Implement "WhatsApp-like" chat and a Social Feed using Supabase Edge Functions.
The database schema (`conversations`, `messages`, `social_posts`, etc.) is
already in place.
...
- **Logic**:
  - Update `status` to 'read' for messages in a conversation where `sender_id`
    != `current_user`.
...
- Add "16. Social Media" section to
