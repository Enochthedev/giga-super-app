...
function scanFunctions() {
    ...
    try {
        ...
        for (const dir of dirs) {
            ...
            // Detect HTTP method
            let method = "POST";
            if (content.includes("req.method === 'GET'") || dir.startsWith("get-") || dir.startsWith("Get-")) {
                method = "GET";
            }
...
function updatePostmanCollection(functions) {
    const collectionPath = path.join(__dirname, '../postman/Giga-API-Collection.postman_collection.json');
    ...
    try {
        ...
        for (const func of missingFunctions) {
            ...
            if (targetFolder) {
                ...
                console.warn(`   ! Could not find folder for module: ${func.module}. Skipping ${func.name}`);
            }
        }
