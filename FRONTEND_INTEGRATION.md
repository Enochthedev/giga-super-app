...
This guide explains how to integrate the Giga Platform API into your frontend application (React, Vue, Mobile, etc.).

## 🚀 Quick Start

### 1. Setup Supabase Client
Ensure you have the Supabase client initialized with your project URL and Anon Key.
...
We use Supabase Edge Functions for all business logic. Use the `functions.invoke` method.
...
Most functions require the user to be logged in. The Supabase client handles this automatically!
Just ensure the user is signed in via `supabase.auth.signInWithPassword(...)` before calling functions.
...
4.  **Book**: `Create-booking` -> Returns `booking_id` and `payment_status: 'pending'`.
...
2.  Set your Environment Variables (`base_url`, `supabase_anon_key`, `supabase_auth_token`).
